...
export async function middlewareAPI(credentials: {email: string, password: string}){
    ...
    if(apiType === "REST"){
        ...
        const user = users.find((user:any) => user.user_email === credentials.email && user.password === credentials.password)
    ...
    if(apiType === "GRAPHQL"){
      ...
      const user = users.find(
        (u: any) => u.userEmail === credentials.email && u.password === credentials.password
      );
