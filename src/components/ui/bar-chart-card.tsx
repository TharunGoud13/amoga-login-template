...
export function BarChartCard({ config,title,
  description,
  value,
  onTitleChange,
  onDescriptionChange, }: BarChartProps) {
  const [chartState, setChartState] = useState<ChartState>({
    title: config.title || "Monthly Sales Comparison",
    description: config.description || "A comparison of online and in-store sales over six months",
...
`)
  const [chartJson, setChartJson] = useState(JSON.stringify(defaultChartData, null, 2))
  ...
  const handleCustomCssChange = useCallback((event: React.ChangeEvent<HTMLTextAreaElement>) => {
    setCustomCss(event.target.value)
  }, [])

  const handleCustomWidgetChange = useCallback((event: React.ChangeEvent<HTMLTextAreaElement>) => {
    setCustomWidget(event.target.value)
  }, [])

  const handleChartJsonChange = useCallback((event: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const actions: Action[] = useMemo(() => config.actions.map(action => ({
  ...
  const executeCustomWidget = useCallback(() => {
    try {
      const CustomWidget = new Function('React', `
      ...
      return <div>Error executing custom widget: {(error as Error).message}</div>;
    }
  }, [customWidget]);
    ...
    <div className="space-y-4 w-full max-w-3xl mx-auto">
      ...
      <Card className="overflow-hidden">
        ...
        <CardFooter className="flex justify-between pb-2">
          {actions.map((action, index) => {
            ...
            return (
              <Button key={index} variant="ghost" size="icon" type="button" onClick={(event) => {
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          onChange={(e) => setChartState(prev => ({ ...prev, customHtml: e.target.value }))}
